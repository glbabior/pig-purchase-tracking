...
> reader. The parser it ships — **Northwind Bank** — reads a statement format that does
> not exist, belonging to a bank that does not exist, so that there is something here you
> can actually run. The parsers for real statements are maintained privately, because a
> parser *is* its issuer: the product names in its regexes identify the account whatever
> the class is called.
>
> So yours will not match, and writing a parser for your own statements is the expected
> path. `NorthwindStatementParser` is the worked example, and the two parts worth copying
> are that a parse is reconciled against the control totals the statement itself prints,
> and that a parser refuses rather than guesses when it cannot tell what it is looking at.
>
> Nothing in the repo requires any file to exist, and a clone with no statements and no
> configuration builds green with nothing skipped.
...
`NorthwindStatementParser` reads statements from a bank that does not exist, in a format
this repository invents, so the app can be run by someone who has none of the author's
data — which is everyone. The format is documented on the parser and generated by
`TestPdfs.NORTHWIND_LINES`; a statement source configured with the parser id
`northwind-demo-pdf` will ingest one.

It is a demonstration, not a toy: it prints control totals and is reconciled against them,
and it refuses an undated statement rather than storing rows against no month. Those are
the two behaviours every parser here is expected to have, and the ones worth copying into
...
included it. **Parser wiring re-checked 2026-08-05**, when dispatch moved from a switch to
discovery (`StatementParserRegistry`) and the parsers for real statements moved to a
private repository; the suite was run with them present and absent. Runtime behavior
against real statements was last checked 2026-07-22, and one reconciliation check is
currently failing on one month's deposit-account statement: its printed debits subtotal
does not match the sum of its debit transactions. The balance itself reconciles, so the
transactions are right and the cross-check is not. That check now lives with its parser,
so it is not part of this repository's suite._
...
- **PDF parsers**, discovered as plugins rather than listed anywhere: a parser
  ...
  [Trying it without any statements](#trying-it-without-any-statements). The parsers
  written against real personal statements are maintained in a separate private
  repository and are not here; the ingest path cannot tell the difference. Every
  parser is covered by a reconciliation test against the control totals its
  statements print
